peerapp = (function () {
  ...
  function connectToServerWithId (peerId) {
    ...
    peer = new Peer(myPeerID, { host: PEER_SERVER, port: PORT, path: '/', secure: false })
    peerCallbacks(peer)
  }
  ...
  function connect (c) {
    ...
    ipcRenderer.on('setUsers',function (event,arg) {
      console.log('arg',arg)
      users = arg
      console.log('peer',c.peer)
      if(!users.includes(c.peer)) {
        console.log("users", users)
        console.log("peer", c.peer)
      } else {
        var friendlist = friends
        var newfriend= friendlist.includes(c.peer)
        if (c.label === 'chat') {
          // c.peer
          myapp.createChatWindow(c.peer, newfriend)

          c.on('data', function (data) {
            console.log(c.peer + ' : ' + data)
            // Append data to chat history
            myapp.appendHistory(c.peer, data,false,true)
          })
  ...
  function fetchOnlinePeers() {
    $.ajax("http://" + PEER_SERVER + ":3000/connected-people")
      .done(function (data) {
        console.log(data)
        if (data.msg == 'Success') {
